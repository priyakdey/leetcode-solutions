...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for cpdomain in cpdomains:
            rep, visited = cpdomain.split(" ")
            parts = visited.split(".")
            for i in range(len(parts)):
                domain = ".".join(parts[i:])
                if domain in rep_map:
                    rep_map[domain] += int(rep)
                else:
                    rep_map[domain] = int(rep)

        result: List[str] = [""] * len(rep_map)
        curr = 0
        for domain, rep in rep_map.items():
            result[curr] = f"{rep} {domain}"
            curr += 1
