...
class Solution:
    def maxSatisfied(
        ...
        sad_customer_count, max_sad_customer_count = 0, 0

        for i in range(minutes):
            if grumpy[i] == 1:
                sad_customer_count += customers[i]

        max_sad_customer_count = sad_customer_count
        start = 0
        end = minutes - 1

        for i in range(minutes, len(customers)):
            if grumpy[i - minutes] == 1:
                sad_customer_count -= customers[i - minutes]
            if grumpy[i] == 1:
                sad_customer_count += customers[i]

            if sad_customer_count > max_sad_customer_count:
                max_sad_customer_count = sad_customer_count
                end = i
                start = i - minutes + 1

        max_satisfied_customer = 0
        for i in range(len(customers)):
            if i not in range(start, end + 1) and grumpy[i] == 0:
                max_satisfied_customer += customers[i]
