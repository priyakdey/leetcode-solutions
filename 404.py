...
class Solution:
    def sumOfLeftLeaves(self, root: Optional[TreeNode]) -> int:
        ...
        return self._sumOfLeftLeaves(root.left, root) + self._sumOfLeftLeaves(root.right, root)
    ...
    def _sumOfLeftLeaves(self, node: TreeNode, parent: TreeNode, sum=0):
        ...
        return sum + self._sumOfLeftLeaves(node.left, node, sum) + self._sumOfLeftLeaves(node.right, node, sum)
